...
void Flash_WriteSector(uint32_t globalAddr, uint8_t *pBuffer, uint16_t byteCount)
{
    ...
    uint16_t leftBytes = byteCount % W25_PAGE_SIZE; //非整数个Page剩余的字节数，即最后一个Page写入的数据
    uint16_t pgCount = byteCount / W25_PAGE_SIZE; //前面整数个Page
    uint8_t *buff = pBuffer;
    for(uint16_t i = 0; i < pgCount; i++) //写入前面pgCount个Page的数据，
    {
        Flash_WriteInPage(globalAddr, buff, W25_PAGE_SIZE); //写一整个Page的数据
        globalAddr += W25_PAGE_SIZE; //地址移动一个Page
        buff += W25_PAGE_SIZE; //数据指针移动一个Page大小
    }
...
// 在 w25flash.c 的基础上添加一个新函数
void Flash_SafeWriteBytes(uint32_t globalAddr, uint8_t *pBuffer, uint16_t byteCount)
{
    // 1. 计算地址和偏移
    uint32_t sectorAddr = globalAddr / FLASH_SECTOR_SIZE * FLASH_SECTOR_SIZE; // 扇区起始地址
    uint16_t offsetInSector = globalAddr % FLASH_SECTOR_SIZE; // 扇区内偏移

    // 假设你有一个扇区大小的缓冲区
    static uint8_t sectorBuffer[FLASH_SECTOR_SIZE];

    // 2. 读取整个扇区到缓冲区 (使用代码二的函数)
    Flash_ReadBytes(sectorAddr, sectorBuffer, FLASH_SECTOR_SIZE);

    // 3. 在内存中修改数据
    memcpy(sectorBuffer + offsetInSector, pBuffer, byteCount);

    // 4. 擦除该扇区 (使用代码二的函数)
    Flash_EraseSector(sectorAddr);

    // 5. 将修改后的整个扇区写回 (使用代码二的函数)
    //    这里需要一个循环来分頁写入
    for(int i = 0; i < (FLASH_SECTOR_SIZE / FLASH_PAGE_SIZE); i++)
    {
        Flash_WriteInPage(sectorAddr + i * FLASH_PAGE_SIZE,
                          sectorBuffer + i * FLASH_PAGE_SIZE,
                          FLASH_PAGE_SIZE);
    }
}
