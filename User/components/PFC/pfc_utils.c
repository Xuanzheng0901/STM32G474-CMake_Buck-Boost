...
float32_t pfc_calc_ideal_duty(float32_t vin_inst, float32_t vout)
{
    ...
    if (vout < 0.1f) return PFC_DUTY_MAX;
...
{
    if (polarity == 0) {
        hhrtim1.Instance->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_B].CMP1xR = 1;
        hhrtim1.Instance->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_B].CMP3xR = 1;
    } else {
        hhrtim1.Instance->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_B].CMP1xR = 1;
